	...
	for (i= m_Modules.begin(); i< m_Modules.end(); i++)
	{
			...
			// erstmal ggf. LUA Filter aufrufen
				...
				if (m_bUseLUAScript==true)
				{
					int z ;
				      /* push functions and arguments */
				     lua_getglobal(L, "pluginFilterPost");  /* function to be called */
				     // start array structure
					   lua_newtable( L );
					   int numCount=1 ;
						for (ResultItemsMap::iterator iter=pRes->m_Items.begin(); iter!=pRes->m_Items.end(); ++iter)
				      ...
				      /* do the call (2 arguments, 1 result) */
				      if (lua_pcall(L, 0, LUA_MULTRET, 0) != 0) {
				      	FILE_LOG(logERROR) << "Fehler beim Aufruf lua dispatcher POST script:" << lua_tostring(L, -1);
				        //error(L, "error running function `f': %s",
				        //         lua_tostring(L, -1));
					  }
				      ...
				      z = lua_tonumber(L, -1);
				      lua_pop(L, 1);  /* pop returned value */
					  FILE_LOG(logDEBUG1) << "lua Result (POST)" << z ;
